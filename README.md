...
| 방법 | 설명 |
|------|------|
| **빌드_실행.vbs 더블클릭** | 검은 창이 뜨면서 빌드 실행. (한글 파일명 때문에 오류 나면 아래 build.bat 사용) |
| **build.bat 더블클릭** | 영문 이름이라 인코딩 오류 없음. "한자로 뜨면서 실행할 수 없다"는 오류가 났을 때 **build.bat**을 더블클릭하세요. |
| **명령 프롬프트에서 실행** | Win+R → `cmd` 입력 → Enter 후, `cd /d "프로젝트폴더경로"` 입력하고 `build.bat` 입력. |
